...
当前正式版为 `2.1.1`，仅支持 Obsidian `1.12.2` 及以上桌面端。项目关系图支持可持久化的项目排序、容器与阶段卡片 Shift 对齐／等间距吸附、局部整理、布局恢复、关系中插和项目颜色编辑；新增 Stage 只放置新节点，既有会话布局与容器顺序保持不变。滴答网络能力默认关闭：普通同步需用户开启，项目投影还需当前版本能力核验与单独确认；既有 API 口令只保留在 SecretStorage。
