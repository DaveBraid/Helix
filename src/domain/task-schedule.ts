...
export const DIDA_CONTRACT_PROBE_VERSION = 13;

/** 本地即可确定的时间能力拒绝；队列必须视为永久能力问题，不得自动重试。 */
export class TaskScheduleWriteError extends Error {
...
export function validateTaskScheduleWrite(
  ...
  if (start !== null && due !== null && start > due) {
    throw new TaskScheduleWriteError("截止时间不能早于开始时间");
  }
  if (mode === "duration" || isPointSchedule(task)) return;
  if (remoteBeforeWrite && sameSchedule(task, remoteBeforeWrite)) return;
  if (mode === "unknown") {
    throw new TaskScheduleWriteError("尚未检测当前滴答账号的时间段能力；请先在 Helix 设置中运行写入合同测试");
  }
  throw new TaskScheduleWriteError("当前滴答账号仅支持单点任务时间，不能提交不同的开始时间和截止时间");
}
...
function validInstant(value: string | null | undefined, label: string): number | null {
  ...
  if (!Number.isFinite(parsed)) throw new TaskScheduleWriteError(`${label}格式无效`);
  return parsed;
}
