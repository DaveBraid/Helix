...
- 新阶段使用稳定展示编号 `helix-stage-code`；保留整数 `helix-sequence` 作唯一高水位和兼容顺序。编号赋值后不随删除或布局重排，唯一例外是单继承显式转为分支时的受控改码。
...
3. 从同一来源首次分支时，共享下一个大号并使用后缀，例如 `1 → 2.1 / 2.2`；后加分支为 `2.3`，删除不复用。若原先已有单继承 `1 → 2`，新增第二后继时必须在同一事务中将原后继改为 `2.1`、新节点为 `2.2`；任一 Markdown/Canvas CAS 失败都不能留下混合编号。
