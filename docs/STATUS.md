# 当前开发状态

最后更新：2026-08-08
当前基线提交：`21ff040 docs: record blocked Dida item contract`
工作树状态：工作树非干净；0.1.0 收口改动尚未提交，详见“本轮改动”。
当前阶段：`0.1.0` 个人预览版收口；普通滴答同步保留，项目到滴答投影不开放，后续主线转向“项目”与“复盘”。

## 本轮目标

- 将当前可稳定使用的普通滴答同步与本地项目／复盘能力固化为 `0.1.0` 个人预览基线。
- 非目标：不重试 items 真实合同，不开放项目投影，不连接远端、打标签或发布 Release，不修改用户滴答或 Vault 数据。

## 当前事实

- 任务、清单、习惯和专注记录以滴答为权威源；任务同步、列表／看板和专注记录读取已有实现，当前可继续使用滴答 App 管理任务。
- Project／Stage Markdown 保存项目身份、状态和正文，专用 Canvas 保存继承／分支／合并关系及布局；`data.json` 不保存项目影子真值。
- 项目投影底层采用“项目父任务 + 阶段行动 `items`”；该账号整组更新会重生成全部 item ID，只有成功响应后的完整语义唯一双射才能原子重映 ledger 与 Stage Markdown。
- 发送前必须核对同父任务全部 Stage 身份、路径、UUID 和 remoteId；缺失、歧义、重排、CAS／ledger 失败或结果未知一律零写入或冻结，禁止猜测与重发。
- Base／Local／Remote 同字段竞争必须逐字段人工解决；凭证只存 SecretStorage，所有远端写入复用唯一队列和 `RemoteWriteGate`。
- `0.1.0` 的项目页与设置页不展示项目投影入口；发布门禁固定关闭后台扫描、激活、分栏创建和投影同步，底层实现与历史诊断仅为后续复核保留。
- 版本兼容基线为 Obsidian 桌面端 `1.12.2`；`manifest.json`、`package.json`、`versions.json` 均为 `0.1.0`。

## 本轮改动

- `src/release-capabilities.ts`、`src/main.ts`：增加并执行 0.1.0 项目投影关闭门禁，旧配置也不会触发后台投影写入。
- `src/ui/helix-view.ts`、`src/ui/settings-tab.ts`：移除项目页同步面板及设置启用／分栏创建入口；普通滴答同步界面不变。
- `tests/workbench-structure.test.ts`：把发布界面与后台门禁约束固化为结构测试。
- `README.md`、本文件：记录个人预览定位、暂未开放能力与版本兼容策略。

## 相关约束

- 权威源、项目投影、全组 ID 重映和未知结果处理见 [ARCHITECTURE.md](ARCHITECTURE.md)。
- 真实测试只能使用唯一合同标记和专用测试清单；不得删除、移动或覆盖用户已有远端对象。
- 合同非幂等写始终只发送一次。30 秒已经是最终等待上限；本轮阻塞后不得继续加时、自动重试或用非唯一复读结果推断成功。
- 2026-08-05 遗留的孤儿合同任务：删除与后续复读均返回 Dida 500。不得宣称已经清理，也不得盲目重试；只能在明确授权、可识别身份和安全预算下另行核对。

## 当前验证

- 基线已通过：66 个测试文件、924 项测试；`typecheck`、`build`、`release:check`、`git diff --check`。
- 本轮已通过针对性测试（3 个文件、39 项）、完整测试（66 个文件、924 项）、`typecheck`、`build`、`release:check` 与 `git diff --check`；均为本地自动化，未访问真实滴答。
- 自动化覆盖不稳定 ID 全组重映、双行动增改删、跨／缺席 Stage、冻结 tombstone、CAS 中断与 verified 重启恢复。
- 最终 30 秒真实合同仍在 items 写入时超时并进入结果未知；后续复读无法唯一证明写入结果。全程零重发，安全清理完成：`cleanup clean`、`remoteArtifacts=false`。
- 当前 `pending=0`、离线队列 `queue=0`、冲突 `conflict=0`、恢复问题 `recovery=0`，且未观察到限流。
- 上述结果是外部真实环境阻塞，不是 items 合同通过；不得宣称生产项目投影可用。当前干净状态也不等于 2026-08-05 孤儿任务已清理。

## 未关闭问题

- 滴答 items 写入在 30 秒内仍不返回，且结果未知后的复读不唯一；真实合同与真实项目投影维持阻塞。
- 2026-08-05 孤儿合同任务的远端状态仍不确定，当前无安全自动清理结论。
- 项目与复盘体验仍未达到最终产品质量，是 `0.1.0` 收口后的核心开发范围。
- 项目投影底层保留但发布门禁关闭；未来重新开放必须先解决 items 合同阻塞，并新增真实安全合同与迁移验收。

## 下一步

1. 完成 0.1.0 针对性测试、完整门禁、独立审查和阶段提交。
2. 在 Obsidian 桌面端确认项目页无同步面板、设置页无项目投影入口，普通滴答同步仍可见可用。
3. 转入项目工作台打磨：Canvas／Markdown 关系、阶段操作、布局与项目行动编辑体验。
4. 随后完善日／周／月／年复盘模板、任务／项目引用及统计闭环。
