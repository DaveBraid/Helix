...
- 文案与内容：父节点未完成时显示直属子任务 `完成数/总数`，全部直属子任务完成后改为绿色勾选；来源和 Stage 状态仍作为次级信息。
- 交互与数据：本地父任务完成圆为只读，详情状态中的“已完成”也不可主动选择；CLI 临时完成全部 Stage 行动后父任务自动完成，重新打开任一行动后父任务自动恢复未完成。自动同步与项目投影均保持关闭，未请求滴答远端。
