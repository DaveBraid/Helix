...
- 本地父任务由直属子任务自动驱动完成状态，支持任意深度任务树与统一任务详情页。
- Helix 独立“正在进行”标记，首页显示前三项及所属项目，并可展开全部。
- 稳定项目、可独立命名的阶段、卡片／看板和内嵌项目关系图。
- 滴答任务与清单双向同步，以及 Helix Project→滴答父子任务的后台单向投影。
- 日、周、月、年结构化复盘模板与自动摘要。
- 事件驱动的趋势、活跃度、XP、成就及周/月挑战。

当前正式版为 `2.1.3`，仅支持 Obsidian `1.12.2` 及以上桌面端。项目关系图支持可持久化的项目排序、容器与阶段卡片 Shift 对齐／等间距吸附、局部整理、布局恢复、关系中插和项目颜色编辑；新增 Stage 只放置新节点，既有会话布局与容器顺序保持不变。滴答网络能力默认关闭：普通同步需用户开启，项目投影还需当前版本能力核验与单独确认；既有 API 口令只保留在 SecretStorage。本版本增加“待记录”阶段、项目状态图标与颜色，并修复 Helix／滴答子任务顺序及测试 Vault 的安全投影恢复。
