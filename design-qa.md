...
- 实现截图：`/private/tmp/helix-task-tree-font-fixed.jpg`，1353 × 768 px；Obsidian 实际窗口、亮色、展开态。
...
- 字体与排版：根据用户实机反馈回归 Helix 任务总览原有密度；标题 13 px、摘要 11 px、元数据 10 px、进度 11 px。标题最多两行，摘要单行截断。
...
- 字号修复同屏对照：`/private/tmp/helix-task-tree-font-comparison.png`，2706 × 768 px；左侧为 16／14／13 px 初版，右侧为 13／11／10 px 修复版，视口与数据完全相同。
- 纯圆进度证据：`/private/tmp/helix-task-tree-circle-fixed-crop.png`；同屏复核 `/private/tmp/helix-task-tree-circle-comparison.png`，2520 × 750 px。进度按钮已锁定 48 × 48 px、等宽 flex basis、50% 圆角、透明底与无主题阴影，悬停和键盘聚焦也不恢复圆角方形底板。
- 比较历史：初次实现满足纯缩进和扁平行结构，但用户实机发现整体字号相对 Helix 其他页面显著放大，记为 P2；字号收紧后，用户继续指出圆形进度会受到主题按钮外观影响，记为 P2。现以高特异性尺寸和外观约束隔离主题注入，实机重载复查为单一圆环，未发现剩余 P0/P1/P2。
