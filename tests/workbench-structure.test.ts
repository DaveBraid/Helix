...
describe("workbench layout and navigation structure", () => {
  ...
  it("persists draggable project order and renders Shift alignment guides for cards and containers", () => {
    ...
    expect(lineage).toMatch(/submitOrder[\s\S]*onReorderProjects\(projectIds\)/);
    expect(lineage).toMatch(/addEventListener\("dragend"[\s\S]*setTimeout\(submitOrder/);
    expect(lineage).not.toContain('setIcon(reorder.createSpan(), "list-ordered")');
    ...
    expect(lineage).toMatch(/bindProjectTitleDrag[\s\S]*event\.shiftKey[\s\S]*lineageShiftBoxAlignment/);
    expect(lineage).toMatch(/structuralNodes\(\)[\s\S]*lineageShiftBoxAlignment/);
    expect(lineage).toMatch(/snapshot\.projects[\s\S]*projectContainerBox[\s\S]*lineageShiftBoxAlignment/);
    expect(lineage).toContain("helix-lineage-alignment-guide");
    expect(lineage).not.toMatch(/marker-(?:start|end)[^\n]*alignment/);
